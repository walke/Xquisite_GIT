...
public class DataBase {
     ...
     */
    public DataBase(Activity activity,int lang)
    {
        tAct=(MainActivity)activity;
        ...
        mDataFile_NO=new File(activity.getExternalFilesDir("DAT_NO").getPath(),"xq_NO.dat");
        try{
            Log.d("DATA", "" + mDataFile_NO.getCanonicalPath());}
        catch (Exception io){}
    ...
    {
        File tmpDF;
        if(lang==0){tmpDF=mDataFile;}
        else{tmpDF=mDataFile_NO;}
        ...
        int size=getDataSize(lang);
        mDataBuffer=new byte[size];
        if(size>0)
        {
            loadDataToBuffer(lang);
        }
     ...
     */
    public int getDataSize(int lang)
    {
        File tmpDF;
        if(lang==0){tmpDF=mDataFile;}
        else{tmpDF=mDataFile_NO;}
        ...
        try
        {
            ...
            FileInputStream fi=new FileInputStream(tmpDF);
            fs=fi.available();
            fi.close();
     ...
     */
    private void loadDataToBuffer(int lang)
    {
        File tmpDF;
        if(lang==0){tmpDF=mDataFile;}
        else{tmpDF=mDataFile_NO;}
     ...
     */
    private void saveBufferToFile(int lang)
    {
        File tmpDF;
        if(lang==0){tmpDF=mDataFile;}
        else{tmpDF=mDataFile_NO;}
        ...
        try
        {
            ...
            FileOutputStream fi=new FileOutputStream(tmpDF);
